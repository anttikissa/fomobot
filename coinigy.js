...
function report() {
	...
	log(
		`60 seconds status: ${counters.orderBookMessages} order book changes, ${
						...
						if (isTrade) {
							...
							if (orderBook) {
								...
								if (orderBook.sells[0]) {
									...
									obData += ` ob sell ${orderBook.sells[0].quantity} at ${
							...
							db.trades
								...
								.catch(err => {
									counters.dbErrors++;
									log(
										`DB error inserting ${exchange} ${tradeId}, will retry soon. Code: ${
											err.code
										}: ${err.sqlMessage}, SQL state: ${err.sqlState}, SQL: ${err.sql}`,
									);
